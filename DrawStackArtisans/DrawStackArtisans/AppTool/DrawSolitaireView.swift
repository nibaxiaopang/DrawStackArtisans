...
let CARDASPECT : CGFloat = 776/1178
let FAN_OFFSET : CGFloat = 0.15

class DrawSolitaireView: UIView {
    ...
    var cardToLayerDictionary : [DrawCard : DrawCardLayer]!
    
    var draggingCardLayer : DrawCardLayer? = nil // card layer dragged (nil => no drag)
    var draggingFan : [DrawCard]? = nil  // fan of cards dragged
    var touchStartPoint : CGPoint = CGPoint.zero
    var touchStartLayerPosition : CGPoint = CGPoint.zero
    
    lazy var solitaire : DrawSolitaire!  = { // reference to model in app delegate
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        return appDelegate.solitaire
    }()
    let numberOfCardsToDeal = 1
    ...
    override func awakeFromNib() {
        ...
        stockLayer.cornerRadius = 4
        stockLayer.backgroundColor =  UIColor(named: "appColor_black_30")?.cgColor
        
        self.layer.addSublayer(stockLayer)
        
        wasteLayer = CALayer()
        wasteLayer.name = "waste"
        wasteLayer.backgroundColor =  UIColor(named: "appColor_black_30")?.cgColor
        wasteLayer.cornerRadius = 8
        self.layer.addSublayer(wasteLayer)
        
        let foundationColor =  UIColor(named: "appColor_black_30")?.cgColor// UIColor(red: 0.0, green: 0.0, blue: 0.5, alpha: 0.3)
        foundationLayers = []
        for i in 0 ..< 4 {
            let foundationLayer = CALayer();
            foundationLayer.name = "foundation \(i)"
            foundationLayer.backgroundColor = foundationColor
            foundationLayer.cornerRadius = 4
            self.layer.addSublayer(foundationLayer)
            foundationLayers.append(foundationLayer)
        }
        
        let tableauColor = UIColor(named: "appColor_black_30")?.cgColor
        tableauLayers = []
        for i in 0 ..< 7 {
            let tableauLayer = CALayer();
            tableauLayer.name = "tableau \(i)"
            tableauLayer.backgroundColor = tableauColor
            tableauLayer.cornerRadius = 4
            self.layer.addSublayer(tableauLayer)
            tableauLayers.append(tableauLayer)
        }
        
        let deck = DrawCard.deck()
        cardToLayerDictionary = [:]
        for card in deck {
            let cardLayer = DrawCardLayer(card: card)
            cardLayer.name = "card"
    ...
    //
    func moveCardsToPosition(_ cards: [DrawCard], position : CGPoint, animate : Bool) {
    ...
    func moveCardLayerToTop(_ cardLayer : DrawCardLayer) {
    ...
    func animateDeal( _ cardLayers : [DrawCardLayer]) {
    ...
    func undoMultiCard(_ cards : [DrawCard]) {
        undoManager?.registerUndo(withTarget: self, handler: { me in
            me.multiCardDeal()
        })
        undoManager?.setActionName("deal cards")
    ...
    func flipCard(_ card : DrawCard, faceUp up : Bool) {
        undoManager?.registerUndo(withTarget: self, handler: {me in
    ...
    func dropCard(_ card : DrawCard, onFoundation i : Int) {
        let cardLayer = cardToLayerDictionary[card]!
        moveCardLayerToTop(cardLayer)
    ...
    func undoDropCard(_ card: DrawCard, fromStack source: DrawCardStack, onFoundation i : Int) {
    ...
    func dropCard(_ card : DrawCard, onTableau i : Int) {
        let cardLayer = cardToLayerDictionary[card]!
    ...
    func undoDropCard(_ card: DrawCard, fromStack source: DrawCardStack, onTableau i : Int) {
    ...
    func dropFan(_ cards : [DrawCard], onTableau i : Int) {
    ...
    func undoDropFan(_ cards : [DrawCard], fromStack source: DrawCardStack, onTableau i : Int) {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let layer = layer {
            if layer.name == "card" {
                let cardLayer = layer as! DrawCardLayer
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if let dragLayer = draggingCardLayer {
            ...
            if let fan = draggingFan {
                for i in 0 ..< 7 {
                    let topCard = solitaire.tableau[i].isEmpty ? nil : solitaire.tableau[i].last
                    var topCardLayer : DrawCardLayer? = nil
                    var targetFrame : CGRect
                    if let topCard = topCard {
